                            ...
                            const Real sleep_ms)
{
  ...
  // config_.topic_imu = "/epson_imu";
  config_.topic_imu = "/razor_imu";
  // config_.radar_yaw_update = 1;
  // config_.filter_min_z = -10;
  // config_.filter_max_z = 10;
  // config_.azimuth_thresh_deg = 90;
  // config_.elevation_thresh_deg = 90;
  config_.min_db = 0;

  x_rio_filter_.configure(config_);
  radar_ego_velocity_.configure(config_);
  yaw_aiding_manhattan_world_.configure(config_);
  ...
  {
    rosbag::Bag source_bag;
    source_bag.open(b, rosbag::bagmode::Read);
    std::cout << b << std::endl;
